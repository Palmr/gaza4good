     ...
     * Twitter consumer key & consumer secret
		 ...
		 */		
		 public function setupApp($consumer_key, $consumer_secret) {
			...
			if(!is_null($this->Session->read('Twitter.Consumer'))) $this->Session->delete('Twitter.Consumer');
      ...
      $this->connectApp($callback_url, 'authenticate');
		...
		public function loginTwitterUser($oauth_token, $oauth_token_secret) {
			if(is_null($this->Session->read('Twitter.User'))) {
				...
				//Save session
				$this->Session->write('Twitter.User', $new_session);				
			} 
		}
		...
		public function logoutTwitterUser() {
			...
			//Destroy session
			if(!is_null($this->Session->read('Twitter.User'))) $this->Session->delete('Twitter.User');
		}
		...
		function initialize(&$controller, $settings = array()) {
			...
			if($this->status() == false) {
				...
				if($this->appStatus() == false) {
					$consumer_session = $this->Session->read('Twiter.Consumer');
				...
				if($this->userStatus() == false) {
					...
					$oauth_session = $this->Session->read('Twitter.User');
